...
class VPGBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size, gamma, lam):
        ...
        # trajectory's remaining return (rewards to go)
    ...
    def end_traj(self, last_val=0):
        ...
        # TD residulas r_t + V^pi(s_{t+1}) - V^pi(s_t)
    ...
    def get(self):
        ...
        data = dict(obs=self.obs_buf, act=self.act_buf, ret=self.ret_buf,
                    tdres=self.tdres_buf, logp=self.logp_buf, rew=self.rew_buf)
...
class Agent:
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            _, logp_a = self.ac.pi.forward(data['obs'], data['act'])
            value = self.ac.v.forward(data['obs'])
            deltas = data['rew'][:-1] + value[1:] - value[:-1]
            # discount TD residuals
            tdres = discount_cumsum(deltas.detach().numpy(), gamma*lam)
            tdres = torch.Tensor((tdres - tdres.mean()) / tdres.std())

            # mean over returns
            mor = np.zeros_like(data['ret'])
            for i in range(len(mor)):
                mor[i] = 1/(i+1) * torch.sum(data['ret'][0:i+1])

            # mean reward baseline
            b = data['rew'].mean()

            #pi_loss = -(logp_a * (data['ret'] - mor)).sum()
            pi_loss = -(logp_a[1:] * tdres).sum()
            pi_loss.backward()
            pi_optimizer.step()
            ...
            for _ in range(100):
                ...
                deltas = data['rew'][:-1] + value[1:] - value[:-1]
                # do the discounting
                tdres = torch.zeros_like(deltas)
                for i in reversed(range(tdres.shape[0])):
                    tdres[i] = gamma*lam * torch.sum(deltas[i:deltas.shape[0]])
                # standardize
                tdres = (tdres - tdres.mean()) / tdres.std()
                v_loss = (0.5 * tdres**2).sum()
    ...
    def get_action(self, obs):
        ...
        """
        # TODO: Implement this function.
        return self.ac.act(torch.Tensor(obs))
